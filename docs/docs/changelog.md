...
- **[NEW]** Connected AI assistants can now stage and run Monte Carlo simulations through MCP — candles resampling (the main overfit-detection mode) and the opt-in trades resampling, plus fetching equity curves and logs. Agent rules default to candles-only with 200 scenarios and document the canonical overfit verdict (compare `original` vs `best_5` / `median`, not `worst_5`) as well as the trades-MC nuance (only `max_drawdown` is informative; sharpe / return are invariant under shuffling).
...
- **[IMPROVEMENT]** `update_config` now recursive-merges the client payload onto the stored config instead of replacing it wholesale. A partial update (e.g. one section) no longer wipes unrelated sections of your settings. The dashboard's behavior is unchanged since it always sends the full settings object; this protects partial-payload callers like the MCP server or scripted clients.
...
- **[IMPROVEMENT]** Added official support for ARM (aarch64) Linux machines in the main Jesse framework — useful for users on AWS Graviton, Ampere, Raspberry Pi 64-bit, and similar hosts. This applies to the main `jesse` package only (backtesting, optimization, Monte Carlo, research); the `jesse-live` plugin does not yet support Linux ARM, so live trading on ARM Linux hosts is not available yet.
- **[IMPROVEMENT]** MCP agent rules (`AGENTS.md`) are now auto-synced on every `jesse run` from the canonical version bundled with the installed Jesse package — existing users no longer need to manually copy or rename `mcp-rules.md` to stay current. Opt out with `jesse run --skip-agent-rules`. See **[MCP agent rules](/docs/mcp/mcp-rules)** for details.
- **[FIX]** Fixed a dashboard crash when opening a Monte Carlo session that was created or run via MCP. The session detail page assumed the full nested `results.trades` / `results.candles` shape that the dashboard itself writes, and crashed (`Cannot set properties of undefined`) for sessions persisted by other clients. The store now re-initializes those slots after loading state, and the template guards its references.
- **[FIX]** Fixed several Monte Carlo MCP bugs surfaced during testing: the runner crashed with `KeyError: 'exchange'` because MCP was forwarding the dashboard's plural `exchanges` config shape (now builds the runner-shaped singular `exchange` dict locally, inferring `spot` vs `futures` from the exchange name); a `pipeline_params: null` value broke the dashboard's session-clone path (defaults are now populated for `moving_block_bootstrap`); and every fresh draft hit a 409 because the run endpoint rejects pre-existing rows (now routed through `/monte-carlo/resume`, which accepts an existing draft).
