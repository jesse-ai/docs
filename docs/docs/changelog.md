...
- **[IMPROVEMENT]** Added official support for ARM (aarch64) Linux machines in the main Jesse framework — useful for users on AWS Graviton, Ampere, Raspberry Pi 64-bit, and similar hosts. CI now runs the full test suite on Linux aarch64 across Python 3.10–3.13. This applies to the main `jesse` package only (backtesting, optimization, Monte Carlo, research); the `jesse-live` plugin does not yet support Linux ARM, so live trading on ARM Linux hosts is not available yet.
