...
`rule_significance_test()` uses a stationary bootstrap that builds a null distribution by resampling random-length contiguous blocks from the rule's zero-centred return series. See the [How it works](/docs/rule-significance-testing/bootstrap) page for the statistical mechanics.
